...
import { isTableKind } from '../ag-ui/types';
...
function sanitizeContentForPrint(html: string, compact: boolean): string {
  ...
  if (compact) {
    // TABLE plus every clinical table kind (medication, vitals, labs, ...)
    doc.querySelectorAll('.scribe-section[data-kind]').forEach((sectionEl) => {
      if (!isTableKind(sectionEl.getAttribute('data-kind') || '')) return;
